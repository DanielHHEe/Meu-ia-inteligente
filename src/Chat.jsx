   ...
   return (
     <div className="border-t border-gray-100 bg-white p-4 flex-shrink-0">
       <div className="max-w-3xl mx-auto">
         <div className="flex items-end gap-3 bg-gray-50 rounded-2xl p-2 border border-gray-200 focus-within:border-emerald-500 focus-within:ring-2 focus-within:ring-emerald-500/20 transition-all">
           <textarea
             value={value}
             ...
             disabled={disabled}
             rows={1}
             className="flex-1 bg-transparent px-3 py-2 text-gray-800 placeholder-gray-400 resize-none focus:outline-none min-h-[44px] max-h-32"
             style={{ height: "auto" }}
           />
           <button
             onClick={onSend}
             disabled={disabled || !value.trim()}
             className="flex-shrink-0 w-11 h-11 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 text-white flex items-center justify-center hover:from-emerald-700 hover:to-teal-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 hover:scale-105 active:scale-95"
           >
             {disabled ? (
 ...
 const ChatInterface = ({ contractType, messages, isTyping, inputValue, setInputValue, onSendMessage }) => {
   ...
   return (
     <div className="flex flex-col h-full">
       {/* Chat Header Info */}
       <div className="bg-gradient-to-r from-emerald-50 to-teal-50 border-b border-emerald-100 p-4 flex-shrink-0">
         <div className="max-w-3xl mx-auto flex items-center justify-between">
           <div className="flex items-center gap-3">
             ...
             <div>
               <h3 className="font-semibold text-gray-900">{contractType.name}</h3>
               <p className="text-sm text-gray-500">Passo 2 de 3 • Coletando informações</p>
             </div>
           </div>
 ...
 const Chat = () => {
   ...
   return (
     <div className="h-screen flex flex-col overflow-hidden bg-gray-50">
       ...
       <main className="flex-1 pt-16 lg:pl-72 flex flex-col min-h-0">
         {currentStep === 1 ? (
